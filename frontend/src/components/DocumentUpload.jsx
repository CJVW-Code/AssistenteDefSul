...
export const DocumentUpload = ({
  ...
  }, [slotFiles, extraFiles, onFilesChange]);
  ...
  const processFile = async (file, slotId = null, customName = null) => {
    ...
    try {
      ...
      if (file.size < 50 * 1024) {
        throw new Error(
          `O arquivo "${file.name}" é muito pequeno (menos de 50KB). Por favor, tire uma foto com melhor qualidade.`,
        );
      }
      ...
      if (isImage) {
        ...
        if (slotId) {
          setPreviewUrls((prev) => ({ ...prev, [slotId]: url }));
        } else {
      ...
      <div key={slot.id} className="relative">
        ...
        {!file ? (
          ...
          <div
            ...
            className={`border-2 border-dashed rounded-xl p-4 flex flex-col items-center justify-center text-center cursor-pointer transition-all h-32 bg-app/30
